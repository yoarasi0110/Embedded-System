...
int read_key(int port)
{
    if(DrvGPIO_GetBit(E_GPA, port) == 0)
    {
        DrvSYS_Delay(8000);  
        if(DrvGPIO_GetBit(E_GPA, port) == 0)
        {
            while(DrvGPIO_GetBit(E_GPA, port) == 0);  
            return 1;
        }
    }
    return 0;
...
void refresh_7seg(uint32_t val)
{
    ...
    close_seven_segment();
    show_seven_segment(digit, num[digit]);
...
int main(void)
{
    ...
    *((volatile uint32_t *)(((( uint32_t)0x50000000) + 0x00000) + 0x100)) = 0x00;;
    ...
    print_lcd(0, "Fast Timer");
    print_lcd(1, "PA0+ PA1- PA2Start");
    ...
    while(1)
    {
        ...
        if(!counting)
        {
            ...
            if(read_key(0)) seconds++;   
            if(read_key(1) && seconds>0) seconds--;   
            if(read_key(2) && seconds>0) counting = 1; 
        }
